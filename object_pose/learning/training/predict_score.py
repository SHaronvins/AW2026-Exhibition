...
class ScorePredictor:
  def __init__(self, amp=True):
    self.amp = amp
    self.run_name = os.getenv("SCORE_RUN_NAME", "2024-01-11-20-02-45")

    def resolve_run_dir(weights_root: Path, run_name: str) -> Path:
      direct = weights_root / run_name
      if (direct / "config.yml").is_file():
        return direct

      # Common layout after unzip: weights/<archive>/<run_name>/config.yml
      matches = list(weights_root.glob(f"**/{run_name}/config.yml"))
      if matches:
        matches.sort(key=lambda p: len(p.parts))
        return matches[0].parent

      raise FileNotFoundError(
        f"Could not find config.yml for run '{run_name}'. Looked under: {weights_root}"
      )

    model_name = 'model_best.pth'
    code_dir = os.path.dirname(os.path.realpath(__file__))

    weights_root = (Path(code_dir) / ".." / ".." / "weights").resolve()
    run_dir = resolve_run_dir(weights_root, self.run_name)
