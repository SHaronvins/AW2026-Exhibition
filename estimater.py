...
class FoundationPose:
  ...
  def reset_object(self, model_pts, model_normals, symmetry_tfs=None, mesh=None):
    ...
    if self.mesh is not None:
      tmp_dir = Path(tempfile.gettempdir()) / 'foundationpose'
  ...
  def make_rotation_grid(self, min_n_views=40, inplane_step=60):
    ...
    if mycpp is None or not hasattr(mycpp, 'cluster_poses'):
      logging.warning(
        "mycpp extension is not available; skipping cluster_poses(). "
        "Tracking may be slower/less stable. Build mycpp for full performance."
      )
    else:
